...
#         #  'args': [],  # Pass DatasetV2File id as an argument
#         result = database_xls_file(task_name, files.id, files.connection_details, files.dataset, files.standardised_file, files.dataset, files.source)
#         print(F"{files} dataset files completed at: ", datetime.now())
#     return f"Fetch data task started at {datetime.now()}"

# def database_xls_file1(file_id, connecton_details, dataset_name, file_path, dataset_id, source):
#     """
...
#                 settings.DATASET_FILES_URL, [dataset_name, source])
#             df.to_excel(file_path)
#             print(F"{file_path} dataset files going to update at: ", datetime.now())
